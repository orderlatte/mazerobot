...
//eswn
public class MazeSolver_StraightFirst implements MazeSolverAlgorithm {
	...
	public byte[] getNext() {
		...
		if (!ds.east) {
			...
			if (!visitedNodes.contains(east)) {
				...
				if (togo != null && directionTogo != null && Arrays.equals(directionTogo, EAST)) {
					togo = east;
					togoValue = EAST;
				}
			}
		...
		if (!ds.south) {
			...
			if (!visitedNodes.contains(south)) {
				...
				if (togo != null && directionTogo != null && Arrays.equals(directionTogo, SOUTH)) {
					togo = south;
					togoValue = SOUTH;
				}
			}
		...
		if (!ds.west) {
			...
			if (!visitedNodes.contains(west)) {
				...
				if (togo != null && directionTogo != null && Arrays.equals(directionTogo, WEST)) {
					togo = west;
					togoValue = WEST;
				}
			}
		...
		if (!ds.north) {
			...
			if (!visitedNodes.contains(north)) {
				...
				if (togo != null && directionTogo != null && Arrays.equals(directionTogo, NORTH)) {
					togo = north;
					togoValue = NORTH;
				}
			}
